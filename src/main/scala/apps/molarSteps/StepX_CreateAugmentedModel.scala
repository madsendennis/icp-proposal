...
object StepX_CreateAugmentedModel {
  implicit val random: Random = Random(1024)

  def main(args: Array[String]): Unit = {
    scalismo.initialize()

    val pcaModel: StatisticalMeshModel = StatisticalModelIO.readStatisticalMeshModel(new File(alignedPath, "models/pca_basic.h5")).get
    ...
    val lowRankGP: LowRankGaussianProcess[_3D, EuclideanVector[_3D]] = LowRankGaussianProcess.approximateGPCholesky(referenceMesh, gp, relativeTolerance = 0.02, interpolator = TriangleMeshInterpolator3D[EuclideanVector[_3D]]())

    val augmentedModel = StatisticalMeshModel.augmentModel(pcaModel, lowRankGP).truncate(1000)

    val rank = augmentedModel.rank

    val outputModelFile = new File(alignedPath, s"models/pca_augmented-crown-${augmentedModel.rank}.h5")
