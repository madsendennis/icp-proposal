...
import api.sampling.{ModelFittingParameters, SamplingRegistration}
...
import scalismo.numerics.{FixedPointsUniformMeshSampler3D, LBFGSOptimizer, Sampler, UniformMeshSampler3D}
import scalismo.registration.{GaussianProcessTransformationSpace, L2Regularizer, MeanHuberLossMetric, MeanSquaresMetric, Registration, Regularizer, TransformationSpace}
...
object GradientBasedRegistration {
  def fitting(model: StatisticalMeshModel, targetMesh: TriangleMesh3D, mySampler: Option[Sampler[_3D]], robustMetric: Boolean=false, numOfIterations: Int, showModel: Option[StatisticalMeshModelViewControls], regWeight: Double = 0.1, modelCoefficients: DenseVector[Double]): DenseVector[Double] = {
    ...
    val transformationSpace = GaussianProcessTransformationSpace(model.gp.interpolate(TriangleMeshInterpolator(model.referenceMesh)))
  ...
  def main(args: Array[String]): Unit = {
    ...
    val modelFile = new File(rawPath, "reference/gp_model_197-components.h5")
...
//    val meshFile = targetMeshes(0)
    targetMeshes.foreach { meshFile =>
      ...
      val dist = (lm1-lm2).norm/1.5
      println(s"Tooth cross distance: ${(lm1-lm2).norm}")
      val sampler = MyLandmarkSampler(model.referenceMesh, centerLandmark, dist, 1000)
      ...
      val finalCoefficients = regWeights.foldLeft(initialCoefficients)((modelCoefficients, regParameters)=>
        fitting(model, targetMesh, Option(sampler), robustMetric=true, numOfIterations = 200, showModel = Option(showModel), regWeight = regParameters, modelCoefficients = modelCoefficients)
      )
      ...
      MeshIO.writeMesh(registered, new File(rawPath, s"computed/crop/premolar2/meshGradientBased2/${targetName}.ply"))
    }
  }
}
